...
// 7.1.5   Code
   ...
   // Init Log Mutex
   pthread_mutex_init(&cm.log_mutex, NULL);
   ...
   pthread_mutex_init(&cm.q_mutex, NULL);
   pthread_cond_init(&cm.q_cv, NULL);
   cm.q_head    = 0;
   cm.q_tail    = 0;
   cm.q_msg_cnt = 0;
   ...
   if (gc.feature & LIN_FEATURE_LOG_TRAFFIC) {
      if ((cm.log = fopen("cm_traffic.txt", "wt")) == NULL) {
      ...
      sprintf(build_date,"%02d.%s.%02d", c_tm->tm_mday, gc.month[c_tm->tm_mon],
   ...
   if (pthread_create(&cm.tid, NULL, cm_thread, NULL)) {
      result = LIN_ERROR_CM;
   }

   // Start CM's Periodic Timer, every 100 mS
   cm_timer = timer_start(100, timer, TIMER_PERIODIC, NULL);

   // Display CM Instance ID
   if (gc.trace & LIN_TRACE_ID) {
      printf("CM Instance, id:devid:handle = %02X:%02X:%02X\n", cm.id, cm.devid, cm.handle);
   }
...
// 7.2.5   Code
   ...
   if (cm.num_objs < CM_MAX_OBJS) {
      ...
      cm.obj[cm.num_objs].timer = timer;
...
// 7.4.5   Code
   ...
   if (ps->msg != NULL) {
      ...
      if (gc.trace & LIN_TRACE_CM) {
         printf("cm_send(), msg_type:srvid:msgid:port:msglen = %02X:%02X:%02X:%1X:%02X\n",
               msg_type, ps->msg->p.srvid, ps->msg->p.msgid,
               ps->msg->h.port, ps->msglen);
      }
...
// 7.5.5   Code
   ...
   // Unlock the CM mutex
   pthread_mutex_unlock(&cm.q_mutex);

   if (gc.trace & LIN_TRACE_ERROR) {
      if (slot == NULL)
         printf("cm_alloc() No Queue Slots Available\n");
   }

   // Trace Exit
   if (gc.trace & LIN_TRACE_CM) {
      if (msg != NULL)
         printf("cm_malloc(), slotid:slot = %02X:%08X\n", msg->h.slot, (uint32_t)slot);
   }
...
// 7.7.5   Code
   ...
   if (gc.trace & LIN_TRACE_ERROR && result == CM_ERR_CRC) {
      printf("cm_crc(), msg : %02X, calc : %02X, len : %d\n", msg->h.crc8, k, msglen);
      dump(pin, msglen, 0, 0);
   }
...
// 7.8.5   Code
   ...
   if (gc.trace & LIN_TRACE_SERVER) {
      printf("cm_msg(), srvid:msgid:port = %02X:%02X:%1X\n",
   ...
   if (cm_msg == MSG(CM_ID_INSTANCE, CM_REG_REQ)) {
      ...
      if (msg->p.flags & CM_REG_OPEN) {
         ...
         if (slot != NULL) {
            ...
            // Device Name
            memcpy(rsp->b.device, gc.dev_str, CM_MAX_DEV_STR_LEN);
            ...
            gc.status |= LIN_STATUS_CONNECTED;
            // report connection
            if (gc.trace & LIN_TRACE_ID) {
               printf("cm_msg() Connected to %s, port %d\n", req->b.device, req->h.port);
            }
         }
      }
      //
      ...
      //
         ...
         if (gc.trace & LIN_TRACE_ID) {
            printf("cm_msg() Disconnected from %s, port %d\n", req->b.device, msg->h.port);
         }
         // No Response
      }
   }
   ...
   //
      ...
      // report connection
      if (gc.trace & LIN_TRACE_ID) {
         printf("cm_msg() Connected to %s, port %d\n", rsp->b.device, rsp->h.port);
      }
      // start ping request timer
   ...
   //
      ...
      if (slot != NULL) {
         ...
         rsp->b.sysid      = gc.sysid;
         rsp->b.stamp      = gc.timestamp;
         rsp->b.devid      = 0;
         rsp->b.num_objs   = 4;
         rsp->b.num_cons   = 2;
         rsp->b.reserved   = 0;
   ...
   //
      ...
      if (gc.trace & LIN_TRACE_ERROR) {
         printf("cm_msg() Unknown Message, msgid:srvid = %02X:%02X\n",
                  msg->p.msgid, msg->p.srvid);
...
/* 7.13.1   Functional Description
   ...
   with timerid. Timer resolution is based on LIN_TICKS_PER_SECOND.
...
// 7.13.5   Code
   ...
   if (k < CM_MAX_TIMERS) {
      ...
      cm.tim[k].count  = time_ms / LIN_TICKS_MS;
      cm.tim[k].reload = time_ms / LIN_TICKS_MS;
   }
   // Timer ID Error
   else {
...
// 7.16.5   Code
   ...
   if (msg == NULL) {
      result = CM_ERR_ROUTE;
      if (gc.trace & LIN_TRACE_ERROR) {
         printf("cm_route() Error : NULL Message\n");
      }
      return result;
   }
   ...
   // Trace the Message before Routing
   if (gc.trace & LIN_TRACE_ROUTE) {
      printf("cm_route() control : \n");
      printf("  dst_cmid:  %02X\n", msg->h.dst_cmid);
      printf("  src_cmid:  %02X\n", msg->h.src_cmid);
      printf("  dst_devid: %1X\n",  msg->h.dst_devid);
      printf("  src_devid: %1X\n",  msg->h.src_devid);
   ...
   if (cm_crc(msg, CM_CHECK_CRC) != CM_OK) {
      result = CM_ERR_CRC;
      if (gc.trace & LIN_TRACE_ERROR) {
         printf("cm_route() Error : Invalid CRC\n");
      }
      cm_free(msg);
      return result;
   }
   ...
   if (cm.port[msg->h.port].media == CM_MEDIA_NONE) {
      ...
      cm_free(msg);
      if (gc.trace & LIN_TRACE_ERROR) {
         printf("cm_route() Error : Media Port Invalid\n");
      }
      cm_free(msg);
      return result;
   }
   ...
   switch (msg->h.event) {
      case CM_EVENT_MSG:
         if (cm.obj[cm_get_handle(cmid)].msg != NULL)
            cm.obj[cm_get_handle(cmid)].msg(msg);
         else
            printf("cm_route() Error : CMID Invalid, CM_EVENT_MSG\n");
         break;
      case CM_EVENT_TIMER:
         if (cm.obj[cm_get_handle(cmid)].timer != NULL)
            cm.obj[cm_get_handle(cmid)].timer(msg);
         else
            printf("cm_route() Error : CMID Invalid, CM_EVENT_TIMER\n");
         break;
      default:
         result = CM_ERR_EVENT_ID;
         if (gc.trace & LIN_TRACE_ERROR) {
            printf("cm_route() Error : Invalid Event ID\n");
         }
         cm_free(msg);
         break;
   }
...
void cm_tx_drop(uint8_t opCode, pcm_msg_t msg) {
...
uint32_t cm_send_msg(uint8_t msg_type, pcm_msg_t msg, pcm_msg_t req,
...
// 7.19.5   Code
   ...
   // Trace Entry
   if (gc.trace & LIN_TRACE_CM) {
      printf("cm_local(), srvid:msgid:flags:status = %02X:%02X:%02X:%02X\n",
               srvid, msgid, flags, status);
   }
   ...
   if (slot != NULL) {
      ...
      msg->h.dst_devid = gc.devid;

      // Source from CM Object
      msg->h.src_cmid  = srvid;
      msg->h.src_devid = gc.devid;
      ...
      // Trace Indication
      if (gc.trace & LIN_TRACE_LOCAL) dump((uint8_t *)msg, msg->h.msglen, 0, 0);
...
// 7.21.5   Code
   ...
   //
      ...
      if (i == CM_MAX_PIPES) result = CM_ERR_NUM_PIPE;
   }
   else
      result = CM_ERR_NUM_PIPE;
...
uint32_t cm_send_reg_req(uint8_t devid, uint8_t port, uint8_t flags,
                         uint8_t *device) {
...
// 7.23.5   Code
   ...
   if (slot != NULL) {
      ...
      // Device Name
      memcpy(msg->b.device, device, CM_MAX_DEV_STR_LEN);
...
// 7.24.5   Code
   ...
   if (msg != NULL) {
      ...
      // Trace Entry
      if (gc.trace & LIN_TRACE_CM) {
         printf("cm_qmsg(), srvid:msgid:port:slot = %02X:%02X:%1X:%02X\n",
               msg->p.srvid, msg->p.msgid, msg->h.port, msg->h.slot);
      }

      // Trace the Message
      if (gc.trace & LIN_TRACE_CMQ) {
         printf("cm_qmsg() : \n");
         printf("  dst_cmid:  %02X\n", msg->h.dst_cmid);
         printf("  src_cmid:  %02X\n", msg->h.src_cmid);
         printf("  dst_devid: %1X\n",  msg->h.dst_devid);
      ...
      pthread_cond_signal(&cm.q_cv);

   }
   else {
      if (gc.trace & LIN_TRACE_ERROR) {
         printf("cm_qmsg() Null Pointer\n");
      }
   }
...
// 7.25.5   Code
   ...
   if (gc.trace & LIN_TRACE_ID) {
      printf("cm_thread() started, data:tid %08X:%lu\n", (uint32_t)data, syscall(SYS_gettid));
   }
   ...
   while (1) {
      ...
      while (cm.q_msg_cnt == 0) {
         ...
         ts.tv_sec += 1;
         pthread_cond_timedwait(&cm.q_cv, &cm.q_mutex, &ts);
...
// 7.26.5   Code
   ...
   if  (((gc.feature & LIN_FEATURE_LOG_TRAFFIC) == 0 ) &&
        ((gc.trace & LIN_TRACE_CM_LOG) == 0)) return;

   if (((gc.feature & LIN_FEATURE_LOG_TRAFFIC) != 0) && cm.log == NULL) return;

   // Lock the CM Log mutex
   pthread_mutex_lock(&cm.log_mutex);

   clock_gettime(CLOCK_REALTIME, &ts);

   now = (uint32_t)(((ts.tv_sec * BILLION) + ts.tv_nsec) / 1000);

   if (cm.last_us == 0) cm.last_us = now;
   delta = (double)(now - cm.last_us);
   if (delta != 0) delta = delta / 100E6;
   delta_secs = (uint32_t)delta;
   delta_us   = (uint32_t)((delta - delta_secs) * 1E4);
   ...
   if (msg->h.dst_cmid != CM_ID_PIPE) {
      ...
      // cm_hdr_t is 8 bytes
      sprintf(line, "%s\t00 : ", dir);
      for (i=0;i<8;i++) {
         sprintf(cat, "%02X", in_u8[i]);
         strcat(line, cat);
      }
      // find cmid and msgid strings
      for (i=0;i<gc.msg_table_len; i++) {
         if ((gc.msg_table[i].cmid  == msg->p.srvid) &&
             (gc.msg_table[i].msgid == msg->p.msgid)) {
            msgid = gc.msg_table[i].msg_str;
            cmid  = gc.msg_table[i].cmid_str;
         }
      }
      sprintf(cat, " %6d  %3d.%04d  %s:%s\n", msg->h.msglen, delta_secs, delta_us, cmid, msgid);
      strcat(line, cat);
      if (gc.trace & LIN_TRACE_CM_LOG)
   ...
   // Pipe Message
      ...
      // find cmid and msgid strings
      for (i=0;i<gc.msg_table_len;i++) {
         if ((gc.msg_table[i].cmid  == pipe->dst_cmid) &&
               (gc.msg_table[i].msgid == pipe->msgid)) {
            msgid = gc.msg_table[i].msg_str;
            cmid  = gc.msg_table[i].cmid_str;
         }
      }
   ...
   // Unlock the CM Log mutex
   pthread_mutex_unlock(&cm.log_mutex);
...
void cm_timer_callback(size_t timer_id, void * user_data) {
