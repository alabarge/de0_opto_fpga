...
// 6.1  Local Function Prototypes
   ...
   static DWORD WINAPI cp_thread(LPVOID data);
...
// 7.1.5   Code
   ...
   if ((rxq.thread = CreateThread(NULL, 0, cp_thread, NULL, 0, &rxq.tid)) == NULL) {
      result = WIN_ERROR_CM;
   }
...
// 7.3.5   Code
   ...
   //
      ...
      gc.error |= WIN_ERROR_PING_TIMEOUT;
      gc.halt   = TRUE;
   }
   //
   //    UNKNOWN TIMER
...
// 7.6.5   Code
   ...
   if (gc.trace & WIN_TRACE_ID) {
      printf("cp_thread() started, tid 0x%X\n", rxq.tid);
   }
