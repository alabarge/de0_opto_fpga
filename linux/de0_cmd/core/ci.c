...
// 7.2.5   Code
   ...
   if (gc.feature & LIN_FEATURE_USE_CI_FILE) {
      // Open and Parse CI File
      if (ci_parse(LIN_CI_FILENAME) != LIN_ERROR_OK) {
      ...
      // Validate Checksum
         ...
         // Validate CI Fields
         if (ci.magic != LIN_CI_MAGIC) {
            // Restore Defaults
            ci_init();
            // Flag the Error
            result = LIN_ERROR_CI_MAGIC;
            if (gc.trace & LIN_TRACE_ERROR) {
