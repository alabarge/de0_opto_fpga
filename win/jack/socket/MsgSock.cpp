// MsgSock.cpp: implementation of the CMsgServer and CMsgClient classes.
...
BOOL CMsgServer::Open(UINT nPort)
{
	return Create(nPort);
}
...
void CMsgServer::SendEx(UINT id, UCHAR *lpBuf, UINT len)
{
...
void CMsgServer::ListenEx(UINT hdrSz, UINT bodyPos, CMsgQueue *pQueue, CWnd *pWnd, UINT id)
{
...
void CMsgServer::OnAccept ( int nErrorCode )
{
	if (nErrorCode == 0) {
...
BOOL CMsgClient::Open(LPCTSTR lpszHostAddress, UINT nHostPort, CWnd *pWnd)
{
   // Open direct
   if (pWnd == NULL) {
      // open
   ...
   // Open using Thread
      ...
      if (Create()) {
         ...
         m_OpenData.pSocket = this;
       	m_RecvData.pWnd    = pWnd;
	      // start the thread
	      AfxBeginThread(OpenThread, &m_OpenData);
      	return TRUE;
      }
      return FALSE;
   }
...
void CMsgClient::SendEx(UCHAR *lpBuf, UINT len)
{
...
void CMsgClient::ListenEx(UINT hdrSz, UINT bodyPos, CMsgQueue *pQueue, CWnd *pWnd, UINT id)
{
	// initialize receive data
...
UINT OpenThread(LPVOID pParam)
{
   DWORD result;
	...
	// get data from thread creator
	OPENDATA *pOpen = (OPENDATA *)pParam;
