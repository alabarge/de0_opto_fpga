...
struct log_t {
    ...
    int use_stdout;
    FILE* out_file;
    void (*log_func) (const int level, const char*);
};
...
/*
*/
static struct log_t log_global_set;
...
void log_reset_state(void)
{
    ...
    cleanup_internal();
    log_global_set.log_func = print_to_syslog;
}
...
int log_set_log_file(const char* filename)
{
    ...
    log_global_set.log_func = print_to_file;
...
void log_generic(const int level, const char* format, va_list args)
{
    ...
    log_global_set.log_func(level, buffer);
}
