...
// 7.1.5   Code
   ...
   if (gc.trace & CFG_TRACE_ID) {
      xlprint("CM Instance, id:devid:handle = %02X:%02X:%02X\n", cm.id, cm.devid, cm.handle);
   }
...
// 7.4.5   Code
   ...
   if (ps->msg != NULL) {
      ...
      if (gc.trace & CFG_TRACE_CM) {
         xlprint("cm_send(), msg_type:srvid:msgid:port:msglen = %02X:%02X:%02X:%1X:%02X\n",
...
// 7.5.5   Code
   ...
   for (i=0;i<CM_MSGQ_SLOTS;i++) {
      if (cmq[cm.q_head].state == CM_Q_IDLE) {
         ...
         if (gc.trace & CFG_TRACE_CM) {
            xlprint("cm_alloc(), slotid:slot = %02X:%08X\n", msg->h.slot, (uint32_t)slot);
         }
         break;
      }
      if (++cm.q_head >= CM_MSGQ_SLOTS) cm.q_head = 0;
   ...
   if (gc.trace & CFG_TRACE_ERROR) {
      if (slot == NULL)
         xlprint("cm_alloc() No Queue Slots Available\n");
   }
...
// 7.7.5   Code
   ...
   if (gc.trace & CFG_TRACE_ERROR && result == CM_ERR_CRC) {
      xlprint("cm_crc(), crc8 : %02X, k : %02X\n", msg->h.crc8, k);
      dump(pin, msglen, 0, 0);
   }
...
// 7.8.5   Code
   ...
   if (gc.trace & CFG_TRACE_SERVER) {
      xlprint("cm_msg(), srvid:msgid:port = %02X:%02X:%1X\n",
   ...
   if (cm_msg == MSG(CM_ID_INSTANCE, CM_REG_REQ)) {
      ...
      if (msg->p.flags & CM_REG_OPEN) {
         ...
         if (slot != NULL) {
            ...
            // report connection
            if (gc.trace & CFG_TRACE_ID) {
               xlprint("cm_msg() Connected to %s, port %d\n", req->b.device, req->h.port);
            }
         }
      }
      //
      ...
      //
         ...
         if (gc.trace & CFG_TRACE_ID) {
            xlprint("cm_msg() Disconnected from %s, port %d\n", req->b.device, msg->h.port);
         }
         // No Response
      }
   }
   ...
   //
      ...
      if (gc.trace & CFG_TRACE_ERROR) {
         xlprint("cm_msg() Unknown Message, msgid:srvid = %02X:%02X\n",
...
// 7.13.5   Code
   ...
   if (k < CM_MAX_TIMERS) {
      ...
      cm.tim[k].count  = time_ms / CFG_TICKS_MS;
      cm.tim[k].reload = time_ms / CFG_TICKS_MS;
   }
   // Timer ID Error
   else {
...
// 7.16.5   Code
   ...
   if (msg == NULL) {
      result = CM_ERR_ROUTE;
      if (gc.trace & CFG_TRACE_ERROR) {
         xlprint("cm_route() Error : NULL Message\n");
      }
      return result;
   }
  ...
  if (gc.trace & CFG_TRACE_ROUTE) {
     xlprint("cm_route() control : \n");
     xlprint("  dst_cmid:  %02X\n", msg->h.dst_cmid);
     xlprint("  src_cmid:  %02X\n", msg->h.src_cmid);
     xlprint("  dst_devid: %1X\n",  msg->h.dst_devid);
     xlprint("  src_devid: %1X\n",  msg->h.src_devid);
     xlprint("  seqid:     %02X\n", msg->h.seqid);
     xlprint("  endian:    %1X\n",  msg->h.endian);
     xlprint("  event:     %1X\n",  msg->h.event);
     xlprint("  keep:      %1X\n",  msg->h.keep);
  ...
  if (cm_crc(msg, CM_CHECK_CRC) != CM_OK) {
     result = CM_ERR_CRC;
     if (gc.trace & CFG_TRACE_ERROR) {
        xlprint("cm_route() Error : Invalid CRC\n");
     }
     cm_free(msg);
     return result;
  }
  ...
  if (cm.port[msg->h.port].media == CM_MEDIA_NONE) {
     ...
     cm_free(msg);
     if (gc.trace & CFG_TRACE_ERROR) {
        xlprint("cm_route() Error : Media Port Invalid\n");
     }
     cm_free(msg);
     return result;
  }
  ...
  switch (msg->h.event) {
     case CM_EVENT_MSG:
        if (cm.obj[cm_get_handle(cmid)].msg != NULL)
           ...
           xlprint("cm_route() Error : CMID Invalid (%02X), CM_EVENT_MSG\n", cmid);
     ...
     case CM_EVENT_TIMER:
        if (cm.obj[cm_get_handle(cmid)].timer != NULL)
           ...
           xlprint("cm_route() Error : CMID Invalid (%02X), CM_EVENT_TIMER\n", cmid);
           dump((uint8_t *)msg, msg->h.msglen, 0, 0);
           cm_free(msg);
        }
        break;
     default:
        ...
        if (gc.trace & CFG_TRACE_ERROR) {
           xlprint("cm_route() Error : Invalid Event ID\n");
        }
        cm_free(msg);
        break;
   }
...
// 7.19.5   Code
   ...
   // Trace Entry
   if (gc.trace & CFG_TRACE_CM) {
      xlprint("cm_local(), srvid:msgid:flags:status = %02X:%02X:%02X:%02X\n",
   ...
   if (slot != NULL) {
      ...
      // Trace Indication
      if (gc.trace & CFG_TRACE_LOCAL) dump((uint8_t *)msg, msg->h.msglen, 0, 0);
...
// 7.24.5   Code
   ...
   if (msg != NULL) {
      ...
      // Trace Entry
      if (gc.trace & CFG_TRACE_CM) {
         xlprint("cm_qmsg(), srvid:msgid:port:slot = %02X:%02X:%1X:%02X\n",
